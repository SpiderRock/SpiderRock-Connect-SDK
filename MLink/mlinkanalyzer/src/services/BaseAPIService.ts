...
class BaseApiService {
  ...
  private async get(url: string): Promise<any> {
    const response = await fetch(url, {
      ...
      console.error(error);
      throw new Error(
        "Something went wrong processing this request. Please ensure your query syntax is valid and try again."
      );
    });
    let jsonResponse;
  ...
  getBaseUrl(): string {
    return this.config.MLINK_ENDPOINT + "?apikey=" + this.config.API_KEY;
  }
