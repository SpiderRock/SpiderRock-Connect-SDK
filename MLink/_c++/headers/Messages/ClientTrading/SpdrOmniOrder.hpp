...
namespace api {
    ...
    #ifndef _alloc_status__OmniAllocStatus__GUARD__
    #define _alloc_status__OmniAllocStatus__GUARD__
    DECL_STRONG_TYPE(alloc_status__OmniAllocStatus, spiderrock::protobuf::api::OmniAllocStatus);
    #endif//_alloc_status__OmniAllocStatus__GUARD__
    ...
    #ifndef _omni_order_number__GUARD__
    #define _omni_order_number__GUARD__
    DECL_STRONG_TYPE(omni_order_number, int64);
    #endif//_omni_order_number__GUARD__
    ...
    class SpdrOmniOrder_PKey {
        public:
        //using statements for all types used in this class
        using omni_order_number = spiderrock::protobuf::api::omni_order_number;

        private:
        omni_order_number m_omni_order_number{};

        public:
        omni_order_number get_omni_order_number() const {
            return m_omni_order_number;
        }
        void set_omni_order_number(const omni_order_number& value)  {
            m_omni_order_number = value;
        }
        //templatized getters and setters
        template <typename T, size_t S = sizeof(T)>
        const auto get() const { static_assert(sizeof(T) == -1, "Unexpected type in call to SpdrOmniOrder_PKey::get()"); return T{}; }  // specializations for valid types are listed below the outer class definition
        template <typename T, size_t S = sizeof(T)>
        void set(const T& value) { static_assert(sizeof(T) == -1, "Unexpected type in call to SpdrOmniOrder_PKey::set()"); }  // specializations for valid types are listed below

        //specializations for set functions for the valid types
        
        void set(const omni_order_number & value) { set_omni_order_number(value); }


        SpdrOmniOrder_PKey() {}

        virtual ~SpdrOmniOrder_PKey() {
        }
        ...
        bool IncludeOmniOrderNumber() const {
            return !(m_omni_order_number == 0);
        }


        size_t ByteSizeLong() const {
            size_t totalSize = 0;
            if ( IncludeOmniOrderNumber()) {
                totalSize += SRProtobufCPP::FieldCodec::LongFieldSize(10,m_omni_order_number);
            }
            return totalSize;
        }
        ...
        void Encode(uint8_t*& dest, uint8_t* max) const {
            if ( IncludeOmniOrderNumber()) {
                dest = SRProtobufCPP::FieldCodec::EncodeLong(dest,10,m_omni_order_number);
            }
        }
            ...
            while (pos < max && (tag = SRProtobufCPP::TagCodec::Decode(pos, max)) != 0) {
                ...
                switch (fieldNumber) {
                    default:
                        ...
                        break;
                    case 10: {m_omni_order_number = SRProtobufCPP::FieldCodec::DecodeLong(pos,max);
                        break;
                    }
                }
            }
    ...
    class SpdrOmniOrder_ChildOrders {
        public:
        //using statements for all types used in this class
        ...
        //templatized getters and setters
        template <typename T, size_t S = sizeof(T)>
        const auto get() const { static_assert(sizeof(T) == -1, "Unexpected type in call to SpdrOmniOrder_ChildOrders::get()"); return T{}; }  // specializations for valid types are listed below the outer class definition
        template <typename T, size_t S = sizeof(T)>
        void set(const T& value) { static_assert(sizeof(T) == -1, "Unexpected type in call to SpdrOmniOrder_ChildOrders::set()"); }  // specializations for valid types are listed below
        ...
        SpdrOmniOrder_ChildOrders() {}

        virtual ~SpdrOmniOrder_ChildOrders() {
        }
        //templatized set functions that can take multiple arguments simultaneously
        template <typename Arg>
        void set_params(Arg && arg) {
    ...
    class SpdrOmniOrder {
        ...
        using pkey = spiderrock::protobuf::api::SpdrOmniOrder_PKey;
        ...
        using alloc_status = spiderrock::protobuf::api::alloc_status__OmniAllocStatus;
        using timestamp = spiderrock::protobuf::api::timestamp;
        using child_orders = spiderrock::protobuf::api::SpdrOmniOrder_ChildOrders;
        ...
        template <typename T, size_t S = sizeof(T)>
        const auto get() const { static_assert(sizeof(T) == -1, "Unexpected type in call to SpdrOmniOrder::get()"); return T{}; }  // specializations for valid types are listed below the class definition

        template <typename T, size_t S = sizeof(T)>
        const auto get(int i) const { static_assert(sizeof(T) == -1, "Unexpected type in call to SpdrOmniOrder::get(int)"); return T{};}  // specializations for valid types are listed below the class definition

        template <typename T, size_t S = sizeof(T)>
        int count() const { static_assert(sizeof(T) == -1, "Unexpected type in call to SpdrOmniOrder::count()"); return 0;}  // specializations for valid types are listed below the class definition
        ...
        void set(const SpdrOmniOrder & value) {
            set(value.m__meta);
            set(value.m_pkey);
        ...
        SpdrOmniOrder() {
            m__meta.set_message_type("SpdrOmniOrder");
        }
        ...
        size_t ByteSizeLong() const {
            ...
            if ( IncludeMlegLegKey()) {
                totalSize += SRProtobufCPP::FieldCodec::StringFieldSize(127,m_mleg_leg_key);
            }
            ...
            totalSize += SRProtobufCPP::FieldCodec::EnumFieldSize(136,static_cast<uint8_t>(static_cast<spiderrock::protobuf::api::OmniAllocStatus>(m_alloc_status)));
            if ( IncludeTimestamp()) {
        ...
        void Encode(uint8_t*& dest, uint8_t* max) const {
            ...
            if ( IncludeMlegLegKey()) {
                dest = SRProtobufCPP::FieldCodec::EncodeString(dest,127,static_cast<string>(m_mleg_leg_key));
            }
            if ( IncludeCumFillQty()) {
            ...
            dest = SRProtobufCPP::FieldCodec::EncodeEnum(dest,136,static_cast<uint8_t>(static_cast<spiderrock::protobuf::api::OmniAllocStatus>(m_alloc_status)));
            if ( IncludeTimestamp()) {
            ...
            while (pos < max && (tag = SRProtobufCPP::TagCodec::Decode(pos, max)) != 0) {
                ...
                switch (fieldNumber) {
                    ...
                    case 136: {if (tagType == SRProtobufCPP::EnumCodec::TagType) {
                            m_alloc_status = static_cast<spiderrock::protobuf::api::OmniAllocStatus>(SRProtobufCPP::FieldCodec::DecodeEnum(pos,max));
                        }
                        break;
                    }
                    case 139: {
    ...
    template<> inline const auto SpdrOmniOrder::get<SpdrOmniOrder::_meta>() const { return SpdrOmniOrder::_meta{ m__meta}; }
    template<> inline const auto SpdrOmniOrder::get<SpdrOmniOrder::pkey>() const { return SpdrOmniOrder::pkey{ m_pkey}; }
    template<> inline const auto SpdrOmniOrder::get<SpdrOmniOrder::omni_accnt>() const { return m_omni_accnt; }
    template<> inline const auto SpdrOmniOrder::get<SpdrOmniOrder::client_firm>() const { return m_client_firm; }
    template<> inline const auto SpdrOmniOrder::get<SpdrOmniOrder::sec_key>() const { return SpdrOmniOrder::sec_key{ m_sec_key}; }
    template<> inline const auto SpdrOmniOrder::get<SpdrOmniOrder::sec_type>() const { return static_cast<uint8_t>(static_cast<spiderrock::protobuf::api::SpdrKeyType>( m_sec_type)); }
    template<> inline const auto SpdrOmniOrder::get<SpdrOmniOrder::order_side>() const { return static_cast<uint8_t>(static_cast<spiderrock::protobuf::api::BuySell>( m_order_side)); }
    template<> inline const auto SpdrOmniOrder::get<SpdrOmniOrder::ssale_flag>() const { return static_cast<uint8_t>(static_cast<spiderrock::protobuf::api::ShortSaleFlag>( m_ssale_flag)); }
    template<> inline const auto SpdrOmniOrder::get<SpdrOmniOrder::position_type>() const { return static_cast<uint8_t>(static_cast<spiderrock::protobuf::api::PositionType>( m_position_type)); }
    template<> inline const auto SpdrOmniOrder::get<SpdrOmniOrder::spdr_source>() const { return static_cast<uint8_t>(static_cast<spiderrock::protobuf::api::SpdrSource>( m_spdr_source)); }
    template<> inline const auto SpdrOmniOrder::get<SpdrOmniOrder::grouping_code>() const { return m_grouping_code; }
    template<> inline const auto SpdrOmniOrder::get<SpdrOmniOrder::base_parent_number>() const { return m_base_parent_number; }
    template<> inline const auto SpdrOmniOrder::get<SpdrOmniOrder::mleg_leg_key>() const { return m_mleg_leg_key; }
    template<> inline const auto SpdrOmniOrder::get<SpdrOmniOrder::cum_fill_qty>() const { return m_cum_fill_qty; }
    template<> inline const auto SpdrOmniOrder::get<SpdrOmniOrder::avg_fill_prc>() const { return m_avg_fill_prc; }
    template<> inline const auto SpdrOmniOrder::get<SpdrOmniOrder::alloc_status>() const { return static_cast<uint8_t>(static_cast<spiderrock::protobuf::api::OmniAllocStatus>( m_alloc_status)); }
    template<> inline const auto SpdrOmniOrder::get<SpdrOmniOrder::timestamp>() const { return m_timestamp; }
    template<> inline const auto SpdrOmniOrder::get<SpdrOmniOrder::child_orders>(int i) const { return SpdrOmniOrder::child_orders{ get_child_orders(i)}; }
    template<> inline int SpdrOmniOrder::count<SpdrOmniOrder::child_orders>() const { return static_cast<int>( m_child_orders.size()); }
    template<> inline const auto SpdrOmniOrder_PKey::get<SpdrOmniOrder_PKey::omni_order_number>() const { return m_omni_order_number; }
    
    template<> inline const auto SpdrOmniOrder_ChildOrders::get<SpdrOmniOrder_ChildOrders::cl_ord_id>() const { return m_cl_ord_id; }
    
    // ostream operators for all classes above, output should adhere to a JSON format

    inline std::ostream& operator<<(std::ostream &o, const SpdrOmniOrder_PKey& m) {
        o << "\"omni_order_number\":" << m.get<SpdrOmniOrder_PKey::omni_order_number>();
        return o;
    }

    inline std::ostream& operator<<(std::ostream &o, const SpdrOmniOrder_ChildOrders& m) {
        o << "\"cl_ord_id\":" << m.get<SpdrOmniOrder_ChildOrders::cl_ord_id>();
        return o;
    }

    inline std::ostream& operator<<(std::ostream &o, const SpdrOmniOrder& m) {
        o << "\"_meta\":{" << m.get<SpdrOmniOrder::_meta>() << "}";
        o << ",\"pkey\":{" << m.get<SpdrOmniOrder::pkey>() << "}";
        o << ",\"omni_accnt\":\"" << m.get<SpdrOmniOrder::omni_accnt>() << "\"";
        o << ",\"client_firm\":\"" << m.get<SpdrOmniOrder::client_firm>() << "\"";
        o << ",\"sec_key\":{" << m.get<SpdrOmniOrder::sec_key>() << "}";
        o << ",\"sec_type\":" << (int64_t)m.get<SpdrOmniOrder::sec_type>();
        o << ",\"order_side\":" << (int64_t)m.get<SpdrOmniOrder::order_side>();
        o << ",\"ssale_flag\":" << (int64_t)m.get<SpdrOmniOrder::ssale_flag>();
        o << ",\"position_type\":" << (int64_t)m.get<SpdrOmniOrder::position_type>();
        o << ",\"spdr_source\":" << (int64_t)m.get<SpdrOmniOrder::spdr_source>();
        o << ",\"grouping_code\":" << m.get<SpdrOmniOrder::grouping_code>();
        o << ",\"base_parent_number\":" << m.get<SpdrOmniOrder::base_parent_number>();
        o << ",\"mleg_leg_key\":\"" << m.get<SpdrOmniOrder::mleg_leg_key>() << "\"";
        o << ",\"cum_fill_qty\":" << m.get<SpdrOmniOrder::cum_fill_qty>();
        o << ",\"avg_fill_prc\":" << m.get<SpdrOmniOrder::avg_fill_prc>();
        o << ",\"alloc_status\":" << (int64_t)m.get<SpdrOmniOrder::alloc_status>();
        {
            std::time_t tt = m.get<SpdrOmniOrder::timestamp>().time_since_epoch().count() / 1'000'000'000;
			struct tm tm1{};
			localtime_s(&tm1, &tt);
        ...
        {
            const char *delim = "{";
            for (int i=0; i<m.count<SpdrOmniOrder::child_orders>(); ++i) {
                o << delim << m.get<SpdrOmniOrder::child_orders>(i) << '}';
                delim = ",{";
            }
        }
        o << "]";
