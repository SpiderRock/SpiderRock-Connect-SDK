...
namespace api {
    ...
    #ifndef _source_time__GUARD__
    #define _source_time__GUARD__
    DECL_STRONG_TYPE(source_time, std::chrono::time_point<std::chrono::system_clock, std::chrono::nanoseconds>);
    #endif//_source_time__GUARD__
    ...
    class StockExchImbalance_PKey {
        ...
        const auto get() const { static_assert(sizeof(T) == -1, "Unexpected type in call to StockExchImbalance_PKey::get()"); return T{}; }  // specializations for valid types are listed below the outer class definition
        template <typename T, size_t S = sizeof(T)>
        void set(const T& value) { static_assert(sizeof(T) == -1, "Unexpected type in call to StockExchImbalance_PKey::set()"); }  // specializations for valid types are listed below
        ...
        StockExchImbalance_PKey() {}

        virtual ~StockExchImbalance_PKey() {
        }
    ...
    class StockExchImbalance {
        ...
        using pkey = spiderrock::protobuf::api::StockExchImbalance_PKey;
        ...
        using source_time = spiderrock::protobuf::api::source_time;
        ...
        source_time get_source_time() const {
        ...
        void set_source_time(const source_time& value)  {
            m_source_time = value;
        }
        ...
        template <typename T, size_t S = sizeof(T)>
        const auto get() const { static_assert(sizeof(T) == -1, "Unexpected type in call to StockExchImbalance::get()"); return T{}; }  // specializations for valid types are listed below the class definition
        ...
        void set(const source_time & value) {
            set_source_time(value);
        }
        void set(const net_timestamp & value) {
            set_net_timestamp(value);
        }

        void set(const StockExchImbalance & value) {
            set(value.m__meta);
            set(value.m_pkey);
            ...
            set(value.m_source_time);
            set(value.m_net_timestamp);
        }

        StockExchImbalance() {
            m__meta.set_message_type("StockExchImbalance");
        }
        ...
        bool IncludeSourceTime() const {
            return (m_source_time.time_since_epoch().count() != 0);
        }
        ...
        size_t ByteSizeLong() const {
            ...
            if ( IncludeSourceTime()) {
                totalSize += SRProtobufCPP::FieldCodec::DateTimeFieldSize(142, m_source_time);
            }
        ...
        void Encode(uint8_t*& dest, uint8_t* max) const {
            ...
            if ( IncludeSourceTime()) {
                dest = SRProtobufCPP::FieldCodec::EncodeDateTime(dest, 142, m_source_time);
            }
            if ( IncludeNetTimestamp()) {
                ...
                switch (fieldNumber) {
                    ...
                    case 142: {
                        if (tagType == SRProtobufCPP::DateKeyCodec::TagType) {
                            m_source_time = SRProtobufCPP::FieldCodec::DecodeDateTime(pos,max);
                        }
                        break;
                    }
                    case 145: {
    ...
    template<> inline const auto StockExchImbalance::get<StockExchImbalance::_meta>() const { return StockExchImbalance::_meta{ m__meta}; }
    template<> inline const auto StockExchImbalance::get<StockExchImbalance::pkey>() const { return StockExchImbalance::pkey{ m_pkey}; }
    template<> inline const auto StockExchImbalance::get<StockExchImbalance::reference_px>() const { return m_reference_px; }
    template<> inline const auto StockExchImbalance::get<StockExchImbalance::paired_qty>() const { return m_paired_qty; }
    template<> inline const auto StockExchImbalance::get<StockExchImbalance::total_imbalance_qty>() const { return m_total_imbalance_qty; }
    template<> inline const auto StockExchImbalance::get<StockExchImbalance::market_imbalance_qty>() const { return m_market_imbalance_qty; }
    template<> inline const auto StockExchImbalance::get<StockExchImbalance::imbalance_side>() const { return static_cast<uint8_t>(static_cast<spiderrock::protobuf::api::ImbalanceSide>( m_imbalance_side)); }
    template<> inline const auto StockExchImbalance::get<StockExchImbalance::continuous_book_clr_px>() const { return m_continuous_book_clr_px; }
    template<> inline const auto StockExchImbalance::get<StockExchImbalance::closing_only_clr_px>() const { return m_closing_only_clr_px; }
    template<> inline const auto StockExchImbalance::get<StockExchImbalance::ssr_filling_px>() const { return m_ssr_filling_px; }
    template<> inline const auto StockExchImbalance::get<StockExchImbalance::indicative_match_px>() const { return m_indicative_match_px; }
    template<> inline const auto StockExchImbalance::get<StockExchImbalance::upper_collar>() const { return m_upper_collar; }
    template<> inline const auto StockExchImbalance::get<StockExchImbalance::lower_collar>() const { return m_lower_collar; }
    template<> inline const auto StockExchImbalance::get<StockExchImbalance::auction_status>() const { return static_cast<uint8_t>(static_cast<spiderrock::protobuf::api::AuctionStatus>( m_auction_status)); }
    template<> inline const auto StockExchImbalance::get<StockExchImbalance::freeze_status>() const { return static_cast<uint8_t>(static_cast<spiderrock::protobuf::api::YesNo>( m_freeze_status)); }
    template<> inline const auto StockExchImbalance::get<StockExchImbalance::num_extensions>() const { return m_num_extensions; }
    template<> inline const auto StockExchImbalance::get<StockExchImbalance::source_time>() const { return m_source_time; }
    template<> inline const auto StockExchImbalance::get<StockExchImbalance::net_timestamp>() const { return m_net_timestamp; }
    template<> inline const auto StockExchImbalance_PKey::get<StockExchImbalance_PKey::ticker>() const { return StockExchImbalance_PKey::ticker{m_ticker}; }
    template<> inline const auto StockExchImbalance_PKey::get<StockExchImbalance_PKey::auction_time>() const { return m_auction_time; }
    template<> inline const auto StockExchImbalance_PKey::get<StockExchImbalance_PKey::auction_type>() const { return static_cast<uint8_t>(static_cast<spiderrock::protobuf::api::AuctionReason>(m_auction_type));}
    template<> inline const auto StockExchImbalance_PKey::get<StockExchImbalance_PKey::exchange>() const { return static_cast<uint8_t>(static_cast<spiderrock::protobuf::api::PrimaryExchange>(m_exchange));}
    
    // ostream operators for all classes above, output should adhere to a JSON format

    inline std::ostream& operator<<(std::ostream &o, const StockExchImbalance_PKey& m) {
        o << "\"ticker\":{" << m.get<StockExchImbalance_PKey::ticker>() << "}";
        {
            std::time_t tt = m.get<StockExchImbalance_PKey::auction_time>().time_since_epoch().count() / 1'000'000'000;
			struct tm tm1{};
			localtime_s(&tm1, &tt);
            o << ",\"auction_time\":\"" << std::put_time(&tm1, "%a %b %e %T %Y") << "\"";
        }
        o << ",\"auction_type\":" << (int64_t)m.get<StockExchImbalance_PKey::auction_type>();
        o << ",\"exchange\":" << (int64_t)m.get<StockExchImbalance_PKey::exchange>();
        return o;
    }

    inline std::ostream& operator<<(std::ostream &o, const StockExchImbalance& m) {
        o << "\"_meta\":{" << m.get<StockExchImbalance::_meta>() << "}";
        o << ",\"pkey\":{" << m.get<StockExchImbalance::pkey>() << "}";
        o << ",\"reference_px\":" << m.get<StockExchImbalance::reference_px>();
        o << ",\"paired_qty\":" << m.get<StockExchImbalance::paired_qty>();
        o << ",\"total_imbalance_qty\":" << m.get<StockExchImbalance::total_imbalance_qty>();
        o << ",\"market_imbalance_qty\":" << m.get<StockExchImbalance::market_imbalance_qty>();
        o << ",\"imbalance_side\":" << (int64_t)m.get<StockExchImbalance::imbalance_side>();
        o << ",\"continuous_book_clr_px\":" << m.get<StockExchImbalance::continuous_book_clr_px>();
        o << ",\"closing_only_clr_px\":" << m.get<StockExchImbalance::closing_only_clr_px>();
        o << ",\"ssr_filling_px\":" << m.get<StockExchImbalance::ssr_filling_px>();
        o << ",\"indicative_match_px\":" << m.get<StockExchImbalance::indicative_match_px>();
        o << ",\"upper_collar\":" << m.get<StockExchImbalance::upper_collar>();
        o << ",\"lower_collar\":" << m.get<StockExchImbalance::lower_collar>();
        o << ",\"auction_status\":" << (int64_t)m.get<StockExchImbalance::auction_status>();
        o << ",\"freeze_status\":" << (int64_t)m.get<StockExchImbalance::freeze_status>();
        o << ",\"num_extensions\":" << m.get<StockExchImbalance::num_extensions>();
        {
            std::time_t tt = m.get<StockExchImbalance::source_time>().time_since_epoch().count() / 1'000'000'000;
			struct tm tm1{};
			localtime_s(&tm1, &tt);
            o << ",\"source_time\":\"" << std::put_time(&tm1, "%a %b %e %T %Y") << "\"";
        }
        o << ",\"net_timestamp\":" << m.get<StockExchImbalance::net_timestamp>();
        return o;
    }
