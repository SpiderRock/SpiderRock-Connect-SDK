...
namespace TimeBoxAnalysis
{
    ...
    {
        private static readonly HashSet<FileQueueWriter> Instances = new();
        ...
        {
            this.basePath = "c:/SRDiagnostics/data";
        ...
        public void WriteRecord(string record)
        {
            ...
            catch (Exception e)
            {
                ...
                SRTrace.Process.TraceError(e, $"FILE.QUEUE.WRITER {tmpFileName}");
            }
        ...
        public void Close()
        {
            ...
            SRTrace.Process.TraceInfo($"FileQueueWriter: CLOSE FILE: [{tmpFileName}]");
        }
        ...
        private StreamWriter GetCreateWriter()
        {
            try
            {
                lock (lockObject)
                {
                    ...
                    string fileName = $"{basePath}/{baseName}.{now.Hour:D2}.{now.Minute:D2}.{now.Day:D2}";
                    ...
                    SRTrace.Process.TraceInfo($"FileQueueWriter: CREATE FILE: ({tmpFileName})");
                    ...
                    for (int i = 0; i < 10; i++)
                    {
                        ...
                        catch (Exception e)
                        {
                            if (i == 9)
                            {
                                ...
                                SRTrace.Process.TraceError(e, $"FileQueueWriter: CREATE EXCEPTION: ({tmpFileName})");
                                return null;
                            }
                            Thread.Sleep(50);
                            continue;
        ...
        private static void RotateFile(StreamWriter writer, string txtFileName, string tmpFileName)
        {
            ...
            try
            {
                ...
                if (tmpFileName != null && txtFileName != null)
                {
                    if (File.Exists(tmpFileName))
                    {
                        SRTrace.Process.TraceInfo($"FileQueueWriter: ROTATE FILE: [{tmpFileName}]");
            ...
            {
                SRTrace.Process.TraceError(e, $"FileQueueWriter: ROTATE EXCEPTION: [{tmpFileName}]");
            }
